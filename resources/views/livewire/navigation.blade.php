<div>
            ...
            <div class="relative flex items-center justify-between h-16">
                ...
                <div class="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                    <a href="/" class="flex-shrink-0 flex items-center">
                        <img class="block lg:hidden h-8 w-auto" src="{{ asset('web/images/jjmm-white.svg') }}" alt="Workflow">
                        
                        <img class="hidden lg:block h-8 w-auto" src="{{ asset('web/images/jjmm-white.svg') }}" alt="Workflow">
                        
                    </a>
                    <div class="hidden sm:block sm:ml-6" x-data="{ open:false }">
                        ...
                        <div class="ml-3 relative" x-data="{ open:false }">
                            <div>
                                <button x-on:click="open = true" type="button" class="bg-gray-800 flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" id="user-menu" aria-expanded="false" aria-haspopup="true">
                                    ...
                                    <img class="lg:block h-8 w-auto rounded-full" src="{{ auth()->user()->profile_photo_url }}" alt="Workflow">
                                </button>
                            </div>
                            ...
                            <div x-show="open" x-on:click.away="open = false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                ...
                                @can('admin.index')
                                    <a href="{{ route('admin.index') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Dashboard</a>
                                @endcan
                                <!-- Authentication -->
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <a href="{{ __('Log Out') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem"
    ...
    <div x-show="open" x-on:click.away="open = false" class="sm:hidden" id="mobile-menu" x-show="open" x-on:click.away="open = false">
