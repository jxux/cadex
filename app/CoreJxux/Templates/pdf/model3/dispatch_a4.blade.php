@php
    ...
    $path_style = app_path('CoreJxux'.DIRECTORY_SEPARATOR.'Templates'.DIRECTORY_SEPARATOR.'pdf'.DIRECTORY_SEPARATOR.'style.css');
...
<table class="full-width border-box mt-10 mb-10">
    ...
    <tr>
        <td colspan="2">P.Partida: {{ $document->origin->location_id }} - {{ $document->origin->address }}, {{ \App\CoreJxux\Templates\FunctionLocation::getLocationName($document->origin->location_id) }}</td>
    </tr>
    <tr>
        <td colspan="2">P.Llegada: {{ $document->delivery->location_id }} - {{ $document->delivery->address }}, {{ \App\CoreJxux\Templates\FunctionLocation::getLocationName($document->delivery->location_id) }}</td>
    </tr>
    </tbody>
</table>
<table class="full-width border-box mt-10 mb-10">
...
<table class="full-width border-box mt-10 mb-10">
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            @php
                $unit_type_description = \App\Models\System\Catalogs\UnitType::find($row->item->unit_type_id);
            @endphp
