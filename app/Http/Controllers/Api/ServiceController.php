...
use App\CoreJxux\Services\Dni\Dni;
use App\CoreJxux\Services\Extras\ExchangeRate;
use App\CoreJxux\Services\Ruc\Sunat;
...
use App\Http\Requests\System\ServiceRequest;
use Exception;
...
class ServiceController extends Controller
{
    ...
    public function exchangeRateTest($date)
    {
        ...
        if($date <= now()->format('Y-m-d')) {
            $ex_rate = \App\Models\System\ExchangeRate::where('date', $date)->first();
            if ($ex_rate) {
                $sale = $ex_rate->sale;
            } else {
                ...
                if ($res) {
                    $ex_rate = \App\Models\System\ExchangeRate::create([
                        'date' => $date,
                    ...
                    $last_ex_rate = \App\Models\System\ExchangeRate::orderBy('date', 'desc')->first();
                    if ($last_ex_rate) {
